...
export function RegisterFlow() {
  ...
  const checkOTPStatus = React.useCallback(async (email: string) => {
    ...
    try {
      ...
      const data: { validated?: boolean; remainingSeconds?: number } = await response.json();
          ...
          <Stepper value={currentStep} onValueChange={onValueChange} onValidate={onValidate}>
            ...
            <div className="p-6 pt-0">
              <div className="flex items-center justify-between w-full">
                ...
                {currentStep === 'passkey' ? (
                  <Button variant="glass" size="default" type="submit">
                    Complete Registration
                  </Button>
                ) : (
                  <StepperNextTrigger asChild>
                    <Button variant="glass" size="default">
                      {currentStep === 'email' && (isSendingOTP ? 'Sending...' : 'Send Code')}
                      {currentStep === 'verify' && (isVerifying ? 'Verifying...' : 'Verify')}
                    </Button>
                  </StepperNextTrigger>
                )}
              </div>
