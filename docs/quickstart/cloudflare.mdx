...
graph TB
    subgraph Frontend["@apps/frontend"]
        ReactComponents["React Components<br/>- IdentityManager.tsx<br/>- Other UI components"]
        Worker["Cloudflare Worker (worker.ts)<br/>- Serves static assets<br/>- Routes API requests to Durable Objects"]
        ReactComponents -->|HTTP Requests| Worker
    end

    subgraph Cloudflare["@pkgs/cloudflare"]
        Counter["CounterDurable<br/>- Simple example"]
        Identity["SonrIdentityDurable<br/>- Identity management<br/>- Wraps @libs/enclave<br/>- Persistent state<br/>- WebSocket support"]
    end

    subgraph Enclave["@libs/enclave"]
        EnclaveClient["EnclaveWorkerClient<br/>- Web Worker<br/>- WASM Runtime<br/>- IndexedDB Storage"]
    end

    Worker --> Counter
    Worker --> Identity
    Identity --> EnclaveClient

    style Frontend fill:#e3f2fd
    style Cloudflare fill:#fff3e0
    style Enclave fill:#f3e5f5
`} />
